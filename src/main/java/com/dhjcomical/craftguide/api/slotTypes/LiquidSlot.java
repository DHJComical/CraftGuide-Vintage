package com.dhjcomical.craftguide.api.slotTypes;
...
public class LiquidSlot implements Slot
{
    ...
    public boolean matches(ItemFilter filter, Object[] data, int dataIndex, SlotType type)
    {
        ...
        if(data[dataIndex] instanceof FluidStack)
        {
            ...
            {
                List<ItemStack> representativeItems = filter.getRepresentativeItems();

                for (ItemStack containerItem : representativeItems) {
                    if (containerItem.isEmpty()) {
                        continue;
                    }

                    IFluidHandlerItem fluidHandler = FluidUtil.getFluidHandler(containerItem);
    ...
    public LiquidSlot setSlotType(SlotType type)
    {
        slotType = type;
        return this;
    }
