...
public class Recipe implements CraftGuideRecipe, CraftGuideRecipeExtra1
{
	...
	public Recipe(Slot[] slots, Object[] items, Renderable background, Renderable backgroundSelected)
	{
		...
		for(int i = 0; i < slots.length; i++)
		{
			...
			if(recipe[i] instanceof ItemStack && slots[i] instanceof ItemSlot && !((ItemSlot)slots[i]).drawQuantity && display != null && display.getCount() > 1)
			{
				ItemStack old = display;

				this.recipe[i] = new ItemStack(old.getItem(), 1, CommonUtilities.getItemDamage(old));
			}
            else if(recipe[i] instanceof ItemStack && slots[i] instanceof com.dhjcomical.craftguide.api.slotTypes.ItemSlot && !((com.dhjcomical.craftguide.api.slotTypes.ItemSlot)slots[i]).drawQuantity && display != null && display.getCount() > 1)
			{
				ItemStack old = display;
	...
	public boolean containsItem(ItemFilter filter, SlotType type)
	{
		if(filter != null)
		{
			for(int i = 0; i < slots.length; i++)
			{
				...
				{
					CraftGuideLog.log("exception trace: com.dhjcomical.craftguide.Recipe.containsItem, while checking slot #" + i + " " + (slots[i] != null? slots[i].getClass() : "null slot"));
